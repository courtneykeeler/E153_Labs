...
%square_and_triangle_analysis Function to analyze what a square and triangle wave looks like
%at the corner and 10x corner frequency.
...
square = square_wave(t_fc, 0.5, fc, harmonics, ones(length(harmonics),1));
triangle = triangle_wave(t_fc, 0.5, fc, harmonics, ones(length(harmonics),1));
...
harmonics = (1:1:999)';
square = square_wave(t_10fc, 0.5, 10*fc, harmonics, ones(length(harmonics),1));
...
legend('Input','Output at corner freq.', 'Output at 10x corner freq. (time scaled)')
set(gca,'FontSize',30)
